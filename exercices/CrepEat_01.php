...
<script>
    ...
    $categories = array(
        ...
        'Sauces' => array(
            'huiledolive' => array('Huile d\'Olive' => 'https://img.icons8.com/color/48/000000/olive-oil.png'), //ERROR with the \'
    ...
    // foreach ($categories as $cagName => $propreties)
    //     shuffle($categories[$cagName]); //Shuffle the order of the buttons
    ...
    galettes.set('La Fermière', new Array('Emmental', 'Poulet Fumé', 'Oeuf'))
    //galettes.set('La Végetariene', new Array('Chèvre', 'Tomate', 'Salade', 'Huile d\'Olive'))
    galettes.set('La Trois Fromages', new Array('Emmental', 'Brie', 'Chèvre'))
    galettes.set('La Popeye', new Array('Oeuf', 'Épinard', 'Chèvre'))
    galettes.set('La Thonthon', new Array('Oeuf', 'Thon', 'Emmental', 'Citron', 'Tomate'))
    galettes.set('La Biquette', new Array('Chèvre', 'Salade', 'Noix', 'Miel'))
    galettes.set('La Nordique', new Array('Saumon Fumé', 'Crême Fraiche', 'Aneth', 'Citron'))
    galettes.set('La Burger', new Array('Viande hachée - Boeuf', 'Emmental', 'Tomate', 'Sauce Burger'))
    galettes.set('La Raclette', new Array('Raclette', 'Pomme de Terre', 'Lardons', 'Crême Fraiche'))
    ...
    function getButtons() { //Getting the button info by ID.
        for (var k = 0; k < inputID.length; k++) {
            document.getElementById(inputID[k]).class = "foodTypeUnselected";
        }
    }
    ...
    function btnclick(btn) { //Function on click of the button.
        ...
        if (btn.class == "foodTypeUnselected") { //if the button is unselected before he click.
            btn.class = "foodTypeSelected";
            btn.setAttribute("class", "foodTypeSelected");
            btn.setAttribute("className", "foodTypeSelected");
        ...
        if (btn.class == "foodTypeSelected") { //if the button is selected before he click.
            btn.class = "foodTypeUnselected";
            btn.setAttribute("class", "foodTypeUnselected");
            btn.setAttribute("className", "foodTypeUnselected");
    ...
    function answerclick(btn) { // Show the solution to the galette.
        if (btn.class == "foodTypeUnselected") { //if the button is unselected before he click.
            ...
            btn.setAttribute("class", "foodTypeSelected");
            btn.setAttribute("className", "foodTypeSelected");
            document.getElementById("answer").value = galettes.get(galettesKey[galettesToFind]) // Input with the ingredient of the galette    
...
<body class="background-main" onload="getButtons();startTheGame();">
                    ...
                    foreach ($categories as $cagName => $propreties) {
                        ...
                        foreach ($propreties as $id => $subPropreties)
                            foreach ($subPropreties as $value => $link) //Creating the buttons.
                                echo "<button id='$id' class='foodTypeUnselected' value='$value' onclick='btnclick(this);'><div id='button-desc' >$value</div> <img id='button-icon' src='$link' /></button>";
